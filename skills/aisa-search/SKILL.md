...
```bash
# Web search
python {baseDir}/scripts/search_client.py web --query "latest AI news" --count 10

# Academic search
python {baseDir}/scripts/search_client.py scholar --query "transformer architecture" --count 10
python {baseDir}/scripts/search_client.py scholar --query "LLM" --year-from 2024 --year-to 2025

# Smart search (web + academic)
python {baseDir}/scripts/search_client.py smart --query "autonomous agents" --count 10

# Full text search
python {baseDir}/scripts/search_client.py full --query "AI startup funding"

# Tavily operations
python {baseDir}/scripts/search_client.py tavily-search --query "AI developments"
python {baseDir}/scripts/search_client.py tavily-extract --urls "https://example.com/article"

# Multi-source search with confidence scoring
python {baseDir}/scripts/search_client.py verity --query "Is quantum computing ready for enterprise?"
```
